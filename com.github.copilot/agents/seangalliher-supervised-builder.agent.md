...
Return exactly one JSON object with `kind` set to `build-report`. Use the
...
Bind the report to the approved `contractHash`, copy its `workflowHash` exactly,
and set `producedBy` to this role's exact resolved selector, not its display
name. For an implemented report, use `status: "implemented"`, a real 40- or
64-character lowercase Git tree hash, at least one changed file, and at least
one check object containing exactly `command`, `outcome: "passed"`, and an
`evidence` object containing `kind`, `locator`, and optional `sha256`; set
`blocker` to `null`. For a blocked report, `testedTreeHash` is `null` and
`blocker` is non-empty. List every changed file, check outcome, evidence locator,
deviation, and unresolved blocker. Never represent a skipped or failed check as
passing.
