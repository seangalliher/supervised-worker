...
`supervised-worker` or the Copilot CLI-qualified
`supervised-worker:supervised-worker`, sourced from the installed
Supervised Worker plugin. Use the host environment or `/env` view when available. Project and user
...
- Stage only approved paths, freeze the candidate, and compute its staged-tree
  ...
  Run `handoff pre-review <contract> <build-report>` and require its hashes,
   ...
   <contract> <build-report>` and require the same hashes. Then invoke
...
- Prefer a reviewer model from a different family than the Builder when the host
   ...
   evidence locators. Supply them to the Reviewer for `modelResolution`; do not
   accept self-attested values. Require the exact configured Reviewer model and
   any configured family separation before accepting `verdict: "clean"`. A host
...
- For each constrained review, use the fresh `reviewAttemptId` and `issuedAt`
   ...
   `observedAt`, `host`, a SHA-256 `sessionHash`, and `source: "host"`. Compute
   the exact receipt SHA-256 and supply its canonical path, hash, and the same
